...
const resentSeacch = catchAsync(async (req, res) => {
  const { searchTerm } = req.body;
  // // Add recent search to the user's recentSearches array
  const result = await UserService.addRecentSearch(searchTerm);
  sendResponse(res, {
    success: true,
    statusCode: StatusCodes.OK,
...
const getResentSeacch = catchAsync(async (req, res) => {
  const { key }: any = req.query;
  const result = await UserService.getRecentSearch(key);
  sendResponse(res, {
    success: true,
    statusCode: StatusCodes.OK,
